...
export class ActivitiesComponent implements OnInit {
  ...
  cargarActividades(): void {
    ...
    this.departmentService.consultarPorNombre(this.NOMBRE_DIRECCION).pipe(
      switchMap(direccion => {
        return this.activityService.consultarPorDireccion(direccion.identificador);
      }),
  ...
  obtenerFechaProgramadaMasCercana(ejecuciones: ActivityExecutionResponse[]): Date | undefined {
    ...
    // Extraer todas las fechas programadas válidas de las ejecuciones
  ...
  calcularEstadoActividad(ejecuciones: ActivityExecutionResponse[]): EstadoActividad {
    ...
    // Verificar si todas las ejecuciones están en estado "Finalizado"
    ...
    // Si al menos una no está finalizada, el estado es pendiente
    return EstadoActividad.PENDIENTE;
  }
  ...
  onActividadCreada(): void {
    // Recargar las actividades cuando se crea una nueva
    this.cargarActividades();
  }
