...
describe('AreaUsersComponent', () => {
  ...
  const mockPaginatedResponse: PaginatedResponse<UserResponse> = {
    contenido: mockUsuarios, totalElementos: 1, totalPaginas: 1, paginaActual: 0
  };
  ...
  describe('obtenerUsuarios', () => {
    ...
    it('should adjust page when content is empty and not first page', () => {
      const emptyResponse: PaginatedResponse<UserResponse> = {
        contenido: [], totalElementos: 10, totalPaginas: 2, paginaActual: 2
      };
    ...
    it('should pass search term and exclude admin role', () => {
      component.searchTerm = 'Juan';
      component.obtenerUsuarios();
      expect(mockUserService.consultarUsuariosPaginado).toHaveBeenCalledWith(
        0, 10, 'Juan', undefined, 'Administrador de dirección'
      );
    });
  });
  ...
  describe('cambiarPagina', () => {
    it('should set page and reload', () => {
      ...
      expect(mockUserService.consultarUsuariosPaginado).toHaveBeenCalledWith(
        2, 10, undefined, undefined, 'Administrador de dirección'
      );
    });
  });
  ...
  describe('ngOnDestroy', () => {
    it('should unsubscribe from all subscriptions', () => {
      ...
      mockUserService.consultarUsuariosPaginado.calls.reset();
      notificationService.notificarUsuarioCreado({});
      expect(mockUserService.consultarUsuariosPaginado).not.toHaveBeenCalled();
    });
  });
});
