...
export class HttpService {
  ...
  public doPostWithOutBodyAndId<T>(
    serviceUrl: string,
    id: number,
    opts: Options
  ): Observable<T> {
  ...
  private createOptions(opts?: Options): Options {
    ...
    if (opts) {
      ...
      if (!opts.headers.get('Content-Type')) {
        const contentType = defaultOpts.headers.get('Content-Type') ?? 'application/json';
