...
describe('UserService', () => {
  ...
  it('should call GET with pagination params for users', () => {
    service.consultarUsuariosPaginado(0, 10).subscribe();
    const req = httpMock.expectOne(r => r.url === `${environment.endpoint}/usuarios/paginado`);
    expect(req.request.method).toBe('GET');
  ...
  it('should pass busqueda and tipoUsuario params when paging users', () => {
    service.consultarUsuariosPaginado(0, 10, 'Juan', 'ESTUDIANTE', 'ADMIN').subscribe();
    const req = httpMock.expectOne(r => r.url === `${environment.endpoint}/usuarios/paginado`);
    expect(req.request.params.get('busqueda')).toBe('Juan');
    expect(req.request.params.get('tipoUsuario')).toBe('ESTUDIANTE');
    expect(req.request.params.get('excluirTipoUsuario')).toBe('ADMIN');
    req.flush({ content: [] });
  });
});
