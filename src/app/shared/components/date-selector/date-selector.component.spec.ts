...
describe('DateSelectorComponent', () => {
  ...
  beforeEach(() => {
    mockActivityService = jasmine.createSpyObj('ActivityService', ['consultarEjecucionesPaginado']);
    mockActivityService.consultarEjecucionesPaginado.and.returnValue(of(mockEjecucionesPaginado as any));
  ...
  describe('ngOnInit', () => {
    it('should load executions when actividad is set', () => {
      ...
      fixture.detectChanges();
      expect(mockActivityService.consultarEjecucionesPaginado).toHaveBeenCalledWith('act-1', 0, 10);
    });
  ...
  describe('ngOnChanges', () => {
    it('should reload executions when actividad changes', () => {
      ...
      expect(mockActivityService.consultarEjecucionesPaginado).toHaveBeenCalledWith('act-2', 0, 10);
    });
  });
  ...
  describe('after loading executions', () => {
    ...
    it('should map executions to sorted fechas', () => {
      ...
      expect(component.fechasOrdenadas[0].ejecucionId).toBe('ej-3'); // May
      expect(component.fechasOrdenadas[1].ejecucionId).toBe('ej-1'); // June
      expect(component.fechasOrdenadas[2].ejecucionId).toBe('ej-2'); // July
    });

    it('should set pagination properties', () => {
      expect(component.totalElementos).toBe(3);
  ...
  describe('cambiarPagina', () => {
    it('should update page and reload', () => {
      ...
      mockActivityService.consultarEjecucionesPaginado.and.returnValue(of({ ...mockEjecucionesPaginado, paginaActual: 2 } as any));
      component.cambiarPagina(2);

      expect(mockActivityService.consultarEjecucionesPaginado).toHaveBeenCalledWith('act-1', 2, 10);
  ...
  describe('trackByFechaId', () => {
    it('should return ejecucionId', () => {
      const fecha = { ejecucionId: 'ej-99', fecha: new Date(), estado: 'Pendiente' as any, actividadId: '' };
      expect(component.trackByFechaId(0, fecha)).toBe('ej-99');
    });
  });
  ...
  describe('obtenerEjecucionPorFecha', () => {
    ...
    it('should find by ejecucionId in map', () => {
      component.actividad = { identificador: 'act-1' } as any;
      fixture.detectChanges();
      ...
      expect(result.identificador).toBe(fecha.ejecucionId);
    });

    it('should find by timestamp fallback', () => {
      component.actividad = { identificador: 'act-1' } as any;
      fixture.detectChanges();
      // Create a fake fecha with matching timestamp but unknown ejecucionId
      const ejDate = (component as any).parseFechaLocal('2025-06-15');
      const customFecha = { ejecucionId: 'unknown-id', fecha: ejDate, estado: 'Pendiente', actividadId: 'act-1' };
  ...
  describe('cargarEjecuciones edge cases', () => {
    ...
    it('should filter out ejecuciones without fechaProgramada', () => {
      mockActivityService.consultarEjecucionesPaginado.and.returnValue(of({
        ...
        totalElementos: 2, totalPaginas: 1, paginaActual: 0
      } as any));
      component.actividad = { identificador: 'act-1' } as any;
      fixture.detectChanges();
